...
class SaleSnapAPIService {
  ...
  getOauthHeadersWithToken = async () => {
    try {
      ...
      this.OAuthTokenExpirationDate = new Date(new Date().getTime() + parseInt(OAuthTokenData.expires_in, 10) * 1000);
    } catch (error) {
  ...
  isOAuthTokenExpired = () => {
    return this.OAuthTokenExpirationDate ? this.OAuthTokenExpirationDate > new Date() : false;
  };
  ...
  getContactsById = async (id: string | number) => {
    ...
    return (await response.json()) as { contact?: { fields?: { all?: { email?: string | null } } } } | undefined;
  };
